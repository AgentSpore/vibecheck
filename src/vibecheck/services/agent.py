...
- КРИТИЧЕСКИ ВАЖНО: ВЕСЬ ТЕКСТ (headline, top_interests, traits, flags, summary) —
  ТОЛЬКО НА РУССКОМ ЯЗЫКЕ. Даже если посты на английском, отвечай ПО-РУССКИ.
  Названия компаний/технологий/имена оставляй как есть (NASA, Python, React и т.п.).
- НЕ используй markdown (`**`, `__`, `##`, `*`, `-`, backticks) — только plain text.
- summary 2-3 абзаца, прямой язык, без канцелярита.

AVATAR (поля в этой секции — строго enum-значения на английском, НЕ переводи их):
...
- КРИТИЧЕСКИ ВАЖНО: весь текст (headline/interests/traits/flags/summary) —
  ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, даже если посты англоязычные. Имена/технологии оставляй как есть.
- НЕ используй markdown (`**`, `__`, `##`, `*`, `-`, backticks) — только plain text.
